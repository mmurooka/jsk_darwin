...
travis_time_start setup.docker
if [ "$(which sudo)" == "" ]; then
  apt-get -q -qq update && apt-get install -y sudo
else
  sudo apt-get -q -qq update
fi
sudo apt-get -q -qq -y install wget apt-transport-https curl build-essential git
...
sudo -E apt-get -y -q -qq install mesa-utils
glxinfo | grep GLX
travis_time_end

su travis -c 'cd $CI_SOURCE_PATH; ls; source ./.ci/travis.sh'
